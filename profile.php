  ...
  <div id="profile-data">
    ...
    <ul class="profile-data">
      <?php if ($_SESSION['userType'] == 'student') { ?>
        ...
        <li><b>Enrollment No: </b>
          <div><?= $_SESSION['enrollment'] ?></div>
        </li>
        <li><b>Faculty No: </b>
          <div><?= $_SESSION['facultyNo'] ?></div>
        </li>
        <li><b>Father's Name: </b>
          <div><?= $_SESSION['fathersName'] ?></div>
        </li>
        <li><b>Phone No.: </b><?php
                              ...
                              ?></li>
        <li><b>Email Id: </b>
          <div><?= $_SESSION['emailId'] ?></div>
        </li>
        <li><b>Date of Birth: </b>
          <div><?= $_SESSION['dateOfBirth'] ?></div>
        </li>
        <li><b>Address: </b>
          <div><?= $_SESSION['address'] ?></div>
        </li>
        <li><b>Join date: </b>
          <div><?= $_SESSION['joinDate'] ?></div>
        </li>
        <li><b>Branch: </b>
          <div><?= $_SESSION['branchName'] ?></div>
        </li>
        <li><b>Semester: </b>
          <div><?= $_SESSION['semester'] ?></div>
        </li>
      <?php } else { ?>
        <li><b>Name: </b>
          <div><?= $_SESSION['name'] ?></div>
        ...
        <li><b>Professor ID: </b>
          <div><?= $_SESSION['professorId'] ?></div>
        </li>
        <li><b>Qualification: </b>
          <div><?= $_SESSION['qualification'] ?></div>
        </li>
        <li><b>Phone No.: </b><?php
                              for ($i = 1; $i <= $_SESSION['phoneCount']; $i++) {
                                echo "<div>" . $_SESSION['phone' . $i] . "</div>";
